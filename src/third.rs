...
pub fn sort<T:Ord>(x:&mut[T],order:&SortOrder)->Result<(),String>{
    //do_sortを呼ぶ代わりにsort_byを呼ぶようにする
    // is_power_of_twoはsort_byが呼ぶのでここからは削除した．
    match *order {
...
fn do_sort<T,F>(x:&mut [T],forward:bool,comparator:&F)
    where F: Fn(&T,&T) -> Ordering{
        if x.len()>1{
            ...
            //xを灰トニックにソートする
            // 第二変数がtrueの時はcomparatorで示される順序でソートする
            do_sort(&mut x[..mid_point], true, comparator);

            //第二引数がfalseのときはcomparatorで示される順序でソートする
...
fn sub_sort<T,F>(x :&mut[T],forward:bool,comparator:&F)
    where F:Fn(&T,&T) -> Ordering{
        if x.len() > 1{
            comapare_and_swap(x,forward,comparator);
            let mid_point = x.len()/2;
...
fn comapare_and_swap<T,F>(x:&mut[T],forward:bool,comparator:&F)
where F:Fn(&T,&T) -> Ordering{
    //比較に先立ち，forward(bool)をOrdering値に変換しておく，
