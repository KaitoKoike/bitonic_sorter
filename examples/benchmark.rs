...
//第三段階のソート関数をseq_sortという別名で使用する
use bitonic_sorter::third::sort as seq_sort;

//第4段階のSort関数をpar_sortという題名で使用する
...
fn main(){
    ...
    if let Some(n) = env::args().nth(1){
        // 文字列型からu32型への変換を試み，失敗したらbitsの束縛する
        let bits = u32::from_str(&n).expect("error parsing argument");
        // 順次ソートと並列ソートを実行する
        run_sorts(bits);
    }else{
        //コマンドライン引数が指定されていなかったら，ヘルプメッセージを表示
        //ステータスコード1で終了する
        eprintln!(
